...
double calibration_factor = -26230;
double butla_pusta = 10.7;
double butla_pelna = butla_pusta + 11.0;
HX711 scale;
...
void setup() {
  ...
  calibration_factor = read_calibration_scale().toInt();
  butla_pusta=read_empty_bottle().toDouble();
...
void createWebServer() {
  ...
  httpServer.on("/set0", []() {
    ...
    save_calibration_scale(httpServer.arg("calibration_scale"));
    save_empty_bottle(httpServer.arg("empty_bottle"));
    if (nr_button > 0) {
...
double get_temperature(int channelNumber, double last_val) {
  double poziom = scale.get_units(10);
  Serial.print(" Pomiar ciężaru: ");
  Serial.print(poziom);
  Serial.println("kg");
  poziom = poziom - butla_pusta;
  poziom = poziom * 100 ;
  poziom = poziom / (butla_pelna - butla_pusta) ; // wyliczanie procentu pojemności butli
  if (poziom < butla_pusta) poziom = 0;
  Serial.print("Poziom w butli : ");

...
String supla_webpage_start(int save) {
  ...
  content += "<div class='w'>";
  content += "<h3>Kalibracja wagi</h3>";
  content += "<i><input name='calibration_scale' value='"; 
  content += String(read_calibration_scale()) ;
  content += "'length=";
  content += MAX_CALIBRATION;
  content += "><label>Wartość kalibracyjna</label></i>";  
  content += "</div>";

  content += "<div class='w'>";
  content += "<h3>Waga pustej putli</h3>";
  content += "<i><input name='empty_bottle' value='"; 
  content += String(read_empty_bottle()) ;
  content += "'length=";
  content += String(sizeof(butla_pusta));
  content += "><label>Waga butli</label></i>"; 
  content += "</div>";

  content += "<button type='submit'>Zapisz</button></form>";
  content += "<br><br>";
  content += "<a href='/search'><button>Kalibracja wagi</button></a>";
    content += "<br><br>";
